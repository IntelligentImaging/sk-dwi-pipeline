...
runs=`find $mpath/plane-dsvrtk -name run-svrtk.sh`
if [[ ! -n $runs ]] ; then
    echo error: run-svrtk.sh not found in the subj/plane-dsvrtk/recon directories
    exit 1
fi
...
for svr in $runs ; do
    ...
    rest=`echo $svr | sed -e 's,.*plane-dsvrtk,plane-dsvrtk,g'`
    date
