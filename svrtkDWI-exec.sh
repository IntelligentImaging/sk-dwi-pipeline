...
runs=`find $mpath/svrtkDWI -name run-svrtk.sh`
if [[ ! -n $runs ]] ; then
    echo error: run-svrtk.sh not found in the subj/svrtkDWI/recon directories
    exit 1
fi
...
for svr in $runs ; do
    ...
    rest=`echo $svr | sed -e 's,.*svrtkDWI,svrtkDWI,g'`
    date
