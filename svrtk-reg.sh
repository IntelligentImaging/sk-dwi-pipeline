...
ID=`basename $CASEDIR`
SVRTK="${CASEDIR}/svrtk"
# b0="${svrtk}/b0/SVRTK/SVRTK-dwi_b0_${ID}.nii.gz"
# b1="${svrtk}/b1/SVRTK/SVRTK-dwi_b1_${ID}.nii.gz"
reg="${SVRTK}/reg"
mkdir -pv $reg
...
function register {
    ...
    tfm_dwit2="${reg}/${b}-t2_${ID}_${mcode}.tfm"
    echo "Register DWI to t2: $b $mcode ($mask)"
    cmd="$cmd $t2_t2 ${SVRTK}/${b}/SVRTK-dwi_${b}_${ID}.nii.gz ${reg}/t2_${b}_${ID}_${mcode}.nii.gz $tfm_dwit2 -t 2 -p 2 --metricName $metric"
    $cmd
    tfm_dwiatlas="${reg}/${b}-atlas_${ID}_${mcode}.tfm"
...
# Check output
REGCOUNT=`find ${reg} -type f -iname t2_b?_\*.nii.gz | wc -w`
