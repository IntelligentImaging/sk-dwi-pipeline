...
svrmask="../mask_svrtk.nii.gz"
b0dir=${svrtk}/b0
b1dir=${svrtk}/b1
runb0=${subj}/${svrtk}/run-svrtk-b0.sh
runb1=${subj}/${svrtk}/run-svrtk-b1.sh
b0SVR="SVRTK-dwi_b0_${id}.nii.gz"
b1SVR="SVRTK-dwi_b1_${id}.nii.gz"
...
for dwi in ${data}/* ; do
    if [[ -d $dwi ]] ; then 
        echo "$dwi - use in reconstruction? (y/n)"
        arg=""
        read arg
        if [[ $arg == "y" ]] ; then
            base=`basename $dwi`
...
for im in `cat $b0list` ; do
	impath=`echo $im | sed 's,.*svrtk/volumes,../volumes,g'`
    echo "$impath \\" >> $runb0
done
